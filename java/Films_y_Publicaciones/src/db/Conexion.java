...
// propiedades para la conexion
    ...
    public void connect(){
        try {
            // jdbc:mysql://localhost/practicasProgra -- url para mysql
// identificar que gestor se utilizara
            if (this.gestor.equalsIgnoreCase("MySql")){ // mysql
                Class.forName("com.mysql.cj.jdbc.Driver");
            } else if(this.gestor.equalsIgnoreCase("MariaDB")) {
...
// conectar con la base de datos
     ...
     */
    public void insertar(String tabla, String nombreModelo, List<Object> datos) {
        ...
        try {
            ...
            sqlInsertar(campos); // crea la sentencia sql que se necesitara
    ...
    private void sqlInsertar(Field[] campos) {
    ...
    private void insertarEnDB(Field[] campos, List<Object> datos) {
        try {

            pst = conexion.getConnect().prepareStatement(this.sql);
            for (int i = 0; i < campos.length; i++) {

                setPrepareStatement(i+1, campos[i], datos.get(i));
            }

            pst.executeUpdate();
        } catch (SQLException e) {
            System.out.println("Error al insertar los datos posible clave primaria repetida");
        }
    }
