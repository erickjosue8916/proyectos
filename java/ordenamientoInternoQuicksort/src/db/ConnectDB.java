...
public class ConnectDB {
    private static ConnectDB conexion = null;

    // propiedades para gestion de la base de datos
    private java.sql.Connection cn; // conexion con la base de datos
    ...
    // constructor
    private ConnectDB(String url, String user, String pass, String gestorDB) {
        this.url = url;
        this.usuario = user;
    ...
    public static ConnectDB getInstance (String url, String user, String pass, String gestorDB) {
        if (conexion == null) {
            conexion = new ConnectDB(url, user, pass, gestorDB);
        }
        return conexion;
    }
...
// conectar con la base de datos
     ...
     */
    public boolean insertar(String tabla, String nombreModelo, List<Object> datos, int inicioCampos) {
        ...
        try {
            ...
            return insertarEnDB(campos, datos, inicioCampos); // inserta los datos en el modelo
    ...
    private boolean insertarEnDB(Field[] campos, List<Object> datos, int n) {
        int indexDB = 1;
        try {
    ...
    public boolean actualizar (String tabla, String nombreModelo, List<Object> datos) {
        ...
        try {
            ...
            return actualizarEnDB(campos, datos); // inserta los datos en el modelo
        } catch (ClassNotFoundException e) {
            System.out.println("Error el modelo no existe");
            return false;
        }
    ...
    private boolean actualizarEnDB(Field[] campos, List<Object> datos){
        try {
            pst = conexion.getConnect().prepareStatement(this.sql);
            for (int i = 1; i < campos.length; i++) {
